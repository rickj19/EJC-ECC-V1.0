...
window.addEventListener('load', async () => {
  const authData = await checkAuth(['ecc'], 'ecc');
  if (authData) {
...
async function loadEccData() {
  try {
    console.log('[ECC] Loading registrations...');
    
    // Fetch ECC registrations
    ...
    console.error('[ECC] Error loading data:', error);
  }
}
...
function renderRegistrations(registrations) {
  ...
  if (!registrations || registrations.length === 0) {
    registrationsList.innerHTML = '<div style="text-align: center; padding: 3rem; color: var(--church-brown-medium);">Nenhuma inscrição encontrada para o ECC.</div>';
    return;
  }
